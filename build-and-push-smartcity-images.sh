#!/bin/bash
# Build and push all Smart City demo Docker images to ECR
...
echo -e "${BLUE}Building and pushing Smart City Docker images to ECR...${NC}"
echo ""

# Get parameters
AWS_REGION="${1:-us-east-1}"
SMARTCITY_SIMULATOR_REPO="$2"
SMARTCITY_DASHBOARD_REPO="$3"

if [ -z "$SMARTCITY_SIMULATOR_REPO" ] || [ -z "$SMARTCITY_DASHBOARD_REPO" ]; then
    echo -e "${RED}Error: Missing repository names${NC}"
    echo "Usage: $0 <aws-region> <smartcity-simulator-repo> <smartcity-dashboard-repo>"
    exit 1
fi
...
echo "Smart City Simulator Repo: $SMARTCITY_SIMULATOR_REPO"
echo "Smart City Dashboard Repo: $SMARTCITY_DASHBOARD_REPO"
echo ""
...
build_and_push "smartcity-simulator" "$BASE_DIR/code/smartcity-simulator" "$SMARTCITY_SIMULATOR_REPO"
build_and_push "smartcity-dashboard" "$BASE_DIR/smartcity-web-dashboard" "$SMARTCITY_DASHBOARD_REPO"

echo -e "${GREEN}✅ All Smart City images built and pushed successfully!${NC}"
