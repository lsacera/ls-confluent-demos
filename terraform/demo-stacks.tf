...
locals {
  ...
  deploy_smartcity = var.enable_smartcity_demo
}
...
module "smartcity_stack" {
  count  = local.deploy_smartcity ? 1 : 0
...
output "smartcity_stack_deployed" {
  description = "Whether Smart City Madrid stack is deployed"
  value       = local.deploy_smartcity
}
...
output "smartcity_stack_outputs" {
  description = "Outputs from Smart City Madrid stack module"
  value       = local.deploy_smartcity ? module.smartcity_stack[0] : null
}
...
module "retail_flink_queries" {
  ...
  depends_on = [
    ...
    time_sleep.wait_for_cdc_topics[0]
  ]
}
...
# Smart City Madrid Flink Queries (7 queries)
